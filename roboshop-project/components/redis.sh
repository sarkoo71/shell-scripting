...
curl -L https://raw.githubusercontent.com/roboshop-devops-project/redis/main/redis.repo -o /etc/yum.repos.d/redis.repo &>>$LOG_FILE_
STAT $?

echo "Install Redis"
yum install redis-6.2.7 -y &>>LOG_FILE_
STAT $?

echo "Update redis configuration"
if [ -f  /etc/redis.conf ]; then
    sed -i -e "s/127.0.0.1/0.0.0.0/g" /etc/redis.conf &>>$LOG_FILE_
elif [ -f /etc/redis/redis.conf ]; then
    sed -i -e "s/127.0.0.1/0.0.0.0/g" /etc/redis/redis.conf &>>$LOG_FILE_
fi
STAT $?

echo "Start Redis"
systemctl enable redis &>>$LOG_FILE_
systemctl start redis &>>$LOG_FILE_
STAT $?
